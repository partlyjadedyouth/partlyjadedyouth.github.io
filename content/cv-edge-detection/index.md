...
`Edge detection`은 위와 같이 어떤 이미지에서 edge만을 검출하는 것을 말한다. 이번 글에서는 `Canny Edge Detector`에 대해서 다루어보려고 한다. [John Canny](https://en.wikipedia.org/wiki/John_Canny)가 1986년에 발표한 이 detector는 edge detection method 중에서 가장 간단하면서도 꽤나 훌륭한 성능을 내는 것으로 알려져 있다.

## 2. Background

### Sobel Operator

Edge를 구하려면 일단 이미지에서 밝기의 변화량을 구해야 한다. 

만약 이미지의 밝기가 1변수 연속함수라면 고등학교 때 배웠던 것처럼 도함수를 구하면 된다. 다변수 연속함수라면 편미분을 해서 gradient를 구하면 된다. 

![gradient](2.jpg)
*&nbsp;*

하지만 이미지는 픽셀 단위로 나누어진 불연속함수이자 행렬이기 때문에 미분을 할 수 없다. 분야에 따라서 다양한 방법으로 불연속함수의 미분을 구하고 있는데, 컴퓨터 비전 분야에서는 **Stanford AI Lab**의 연구원 [Irwin Sobel](https://en.wikipedia.org/wiki/Irwin_Sobel)이 1968년에 제안한 `Sobel Operator`라는 $3 \times 3$ 행렬을 이용해서 `gradient`를 구하는 경우가 많다.
...
따라서 Sobel operator를 적용하기 전에는 이미지에 [Gaussian smoothing](https://partlyjadedyouth.github.io/cv-gaussian-smoothing/)을 적용하여 denoising을 해주어야 한다. 
...
LoG는 이렇게 생겼다. 마치 멕시코인들이 쓰는 모자(솜브레로)같다 하여 Mexican Hat Function이라고도 부른다.
...
이제 본격적으로 Canny edge detector의 알고리즘에 대해 Python 코드를 보면서 알아보도록 하자.
...
img = cv2.imread('pumpkin.jpg', cv2.IMREAD_GRAYSCALE)
...
`cv2.Sobel`을 이용해 gradient image를 구한다. 여기서 유의해야 할 점은, OpenCV의 Sobel operator는 normalize되지 않는다는 것이다. 즉, `blurred`의 최댓값과 `sx`, `sy`의 최댓값이 다르다는 것이다. 따라서 이들을 같게 만들어주는 작업이 필요하다.
...
이미지는 불연속함수이기 때문에, `p`나 `r`이 위 그림처럼 정확히 어떤 픽셀을 가리키지 않고 어떤 두 픽셀 사이의 지점이 될 수도 있다. 
...
이러한 단점을 보완하기 위해 Canny edge detector에서는 `hysteresis edge tracking`이라는 방법을 사용한다.

총 2개의 threshold를 사용하는데, 그 중 값이 큰 것을 `high`, 작은 것을 `low`라고 하자. 만약 어떤 픽셀의 값이,

* `high`보다 크면 edge이다.
* `high`보다 작지만 `low`보다는 크다면 주변 픽셀들을 확인한다.
  * 만약 주변에 edge인 픽셀이 있다면 edge이다.
  * 없다면 edge가 아니다.
* `low`보다 작다면 edge가 아니다.
...
전체 코드는 아래 repository에서 확인할 수 있다.
